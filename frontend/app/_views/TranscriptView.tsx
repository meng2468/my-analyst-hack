...
export default function TranscriptView({
    ...
    for (let i = 0; i < messages.length; ++i) {
        ...
        renderedMessages.push(
            <div key={i} className={`flex ${justifyClass}`}>
                <div className={`text-xl rounded-lg px-4 py-3 max-w-[75%] whitespace-pre-line text-base shadow-sm ${messageClass}`}>
                    {renderWithLineBreaksFromString(msg.content)}
                </div>
            </div>
        );
            ...
            return (
                ...
                >
                    ...
                    {expanded && (
                        <>
                            <div
                                className="fixed inset-0 bg-white/50 backdrop-blur-lg transition-opacity duration-100"
    ...
    return (
        <div className="flex w-full flex-col items-center justify-center bg-white/15 backdrop-blur-lg border border-white p-6 rounded-lg relative">
            <div className="flex w-[1200px] gap-6 mx-auto items-start">
                {/* Left: Transcript Area */}
                <div className="flex-1 flex flex-col items-center">
                    <div
                        className="rborder-gray-100 shadow 
              mb-0 overflow-y-auto
            ...
            <div className="absolute bottom-4 right-4 flex gap-2 z-10">
                <Button onClick={handleDownload} size="icon" variant="default"
                    className="rounded-full cursor-pointer" title="Download">
                    <Download size={20} />
                </Button>
                <Button onClick={handleGenerateSummary} size="icon" variant="default"
                    className="rounded-full cursor-pointer" title="Generate Summary">
                    <Database size={20} />
                </Button>
                <Button onClick={() => setStep(0)} size="icon" variant="secondary"
                    className="rounded-full cursor-pointer" title="Restart">
                    <RotateCcw size={20} />
                </Button>
            </div>
            {/* Agent Progress */}
